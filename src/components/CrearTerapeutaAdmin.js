...
const CrearTerapeutaAdmin = (props) => {
  ...
  async function CrearTerapeuta(){
    const url = process.env.REACT_APP_API_HOST+'/api/terapists/putTerapist';
    const body = {
      name: name,
      lastName:lastName,
      // secondLastName:'Soto',
    ...
    fetch(url,{
      ...
      if(res.status === 200){
        Swal.fire({
          ...
          text:'¡Se creó el terapeuta con éxito!',
          icon:'success',
        ...
        Swal.fire({
          title:'Error!',
          text:'¡No se pudo crear el terapeuta!',
          icon:'error',
          confirmButtonText:'Ok',
          // showCancelButton:true,
          // cancelButtonText:'No'
...
export default CrearTerapeutaAdmin